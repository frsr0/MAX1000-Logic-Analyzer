-- Testbench for continuous buffer rotation with debug
library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;

entity tb_continuous2 is
end tb_continuous2;

architecture sim of tb_continuous2 is
  ...
  component Fast_Logic_Analyzer_SDRAM is
    ...
    port (CLK: in std_logic; CLK_150: out std_logic;
      ...
      sdram_cas_n: out std_logic; sdram_dq: inout std_logic_vector(15 downto 0);
      sdram_dqm: out std_logic_vector(1 downto 0); sdram_ras_n: out std_logic;
      sdram_we_n: out std_logic; sdram_cke: out std_logic;
      sdram_cs_n: out std_logic; sdram_clk: out std_logic;
...
begin
  ...
  UUT : Fast_Logic_Analyzer_SDRAM
    ...
    port map (CLK => clk, CLK_150 => pclk,
      Rate_Div => rate_div, Samples => samples, Start_Offset => 0,
      Run => run, Full => full, Inputs => inputs, Address => address,
      Outputs => outputs, sdram_addr => sdram_addr, sdram_ba => sdram_ba,
  ...
  begin
    ...
    if run = '1' then
      cyc := cyc + 1;
      -- Check status(3) = full_i
      if status(3) = '1' and full = '0' then
        report "MISMATCH: status(3)=" & std_logic'image(status(3)) & " full=" & std_logic'image(full) severity warning;
      end if;
    end if;
  end process;
  ...
  process
  begin
    armed <= '1'; continuous_mode <= '1'; fast_mode <= '1';
    wait for 20 ns;
    run <= '1';
    ...
    report "Full! status(3)=" & std_logic'image(status(3)) &
    ...
    -- Ack buffer 0
    report "Acking..." severity note;
    buffer_ack <= "001";
    wait until rising_edge(clk);
    buffer_ack <= "000";
    wait for 40 ns;

    report "After ack: full=" & std_logic'image(full) &
      " status(3)=" & std_logic'image(status(3)) severity note;

    wait for 200 ns;
    report "After 200ns: full=" & std_logic'image(full) &
      " status(3)=" & std_logic'image(status(3)) severity note;
