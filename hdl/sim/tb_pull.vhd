-- Testbench: pull-up/down force floating inputs to known state
library ieee;
...
architecture sim of tb_pull is
  ...
  signal inputs : std_logic_vector(15 downto 0) := (others => 'X');
  ...
  signal pulldown_mask : std_logic_vector(7 downto 0) := (others => '0');
  signal pullup_mask : std_logic_vector(7 downto 0) := (others => '0');
  ...
  component Fast_Logic_Analyzer_SDRAM is
    ...
    port (CLK: in std_logic; CLK_150: out std_logic;
      ...
      Pulldown_Mask: in std_logic_vector(7 downto 0):=(others=>'0');
      Pullup_Mask: in std_logic_vector(7 downto 0):=(others=>'0'));
  end component;
...
begin
  ...
  UUT: Fast_Logic_Analyzer_SDRAM
    ...
    port map (CLK=>clk, CLK_150=>pclk, Rate_Div=>rate_div, Samples=>samples,
      ...
      Pulldown_Mask=>pulldown_mask, Pullup_Mask=>pullup_mask);

  process
  begin
    report "=== PULL CONTROL TEST ===" severity note;

    armed <= '1'; fast_mode <= '1';
    run <= '1';
    wait until rising_edge(full);
    report "Full asserted at " & time'image(now) severity note;

    -- Read back captured data
    address <= 0;
    wait for 20 ns;
    report "No pull: Outputs(0) = 0x" & to_hstring(outputs) severity note;

    -- Ack buffer 0
    buffer_ack <= "001";
    wait until rising_edge(clk);
    buffer_ack <= "000";
    wait until falling_edge(full) for 50 us;

    -- Enable all pull-downs: floating 'X' inputs forced to 0
    pulldown_mask <= x"FF";
    wait for 50 ns;

    -- Re-capture with pull-downs
    run <= '0'; wait for 100 ns; run <= '1';
    wait until rising_edge(full) for 100 us;
    if full = '1' then
      address <= 0;
      wait for 20 ns;
      report "Pull-down: Outputs(0) = 0x" & to_hstring(outputs) & "  (expect 0x0000)" severity note;
    else
      report "TIMEOUT waiting for Full (pull-down)" severity note;
    end if;

    -- Ack then switch to pull-up
    buffer_ack <= "001";
    wait until rising_edge(clk);
    buffer_ack <= "000";
    wait until falling_edge(full) for 50 us;
    ...
    if full = '1' then
      address <= 0;
      wait for 20 ns;
      report "Pull-up:  Outputs(0) = 0x" & to_hstring(outputs) & "  (expect 0xFFFF)" severity note;
    else
      report "TIMEOUT waiting for Full (pull-up)" severity note;
    end if;

    -- Ack then disable all (back to floating)
    buffer_ack <= "001";
    wait until rising_edge(clk);
    buffer_ack <= "000";
    wait until falling_edge(full) for 50 us;
