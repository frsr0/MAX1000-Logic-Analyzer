...
--
-- The capture compressor first produces packed delta words, then the output
-- stream is buffered and fed into the exact run-length encoder. This keeps the
-- public readback contract to one merged codec while preserving the original
-- delta packer's compact representation for flat/slow waveforms.
--
...
architecture rtl of delta_rle_compressor is
  -- Delta packing emits short six-word bursts every 16 samples. 16 entries
  -- covers the worst alternating-word burst plus the RLE two-cycle emit gap;
  -- the former 256-entry queue added unnecessary control/placement pressure.
  ...
  begin
    if rising_edge(clk) then
      if rst = '1' then
        ...
        if delta_mode = '1' and delta_valid = '1' then
          if count_v < FIFO_DEPTH then
            fifo(wr_ptr_v) <= delta_data;
            if wr_ptr_v = FIFO_LAST then
              wr_ptr_v := 0;
            else
              wr_ptr_v := wr_ptr_v + 1;
            end if;
        ...
        fifo_wr_ptr <= wr_ptr_v;
        fifo_rd_ptr <= rd_ptr_v;
        fifo_count <= count_v;

        if flush_request = '1' and count_v = 0 and delta_busy = '0' then
          flush_armed <= '1';
        end if;

        if flush_armed = '1' and rle_flush = '1' then
          flush_issued <= '1';
        end if;
