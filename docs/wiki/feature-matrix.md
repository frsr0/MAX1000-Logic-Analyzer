...
| Narrow packed capture | Narrow live mode control and decoder | 16 time samples from one selected channel per word | **HW** at 200.4 MHz; maintained simulation coverage |
| Analog-fast | Analog-fast mode and analog waveform | One MAX10 ADC lane | **HW**; ADC1/AIN3 profile and physical analog checks |
| Maximum analog | Maximum-analog single/live modes | Packed MSO path, ADC1-ADC4 (`AIN3`, `AIN1`, `AIN4`, `AIN6`) | **HW**; four distinct lanes at about 24 kS/s each |
| Mixed-signal capture | Mixed mode and analog/digital panels | Raw five-byte frames: 16 digital bits plus two ADC results | **HW**; 500,000 packed words |
...
|---|---|---|---|
| UART | UART decoder, parity/framing/break | Bit Banger UART, one-shot or live repeat | **HW** loopback, live capture, and 1,200-115,200 baud sweep |
...
| PWM | Pulse/frequency/duty decoder | Bit Engine/Bit Banger templates and FPGA repeat | **HW** loopback and repeat path |
...
| CSV/JSON/VCD/PulseView-VCD/NPZ/HTML/PDF export | [Export Formats](backend/export-formats.md) | **SW** export/API and Playwright coverage |
...
| Exact generator rate reporting | [Generator Controller](backend/generator-controller.md) | **HW/SW**; metadata-selected 16/24-bit divider and periodic output frequency |
| Live generator output | [Generator Routing](generator-routing.md) | **HW** for UART/RS-485/Bit Banger during rolling capture |
...
| Full build | `FAST_SPEED=true`, `FAST_RAW_BUILD=false`, Quartus 25.1, seed 10 |
| Timing | Slow-85C setup: `fast_clk` **+0.083 ns**, `sdram_core_clk` **+0.111 ns**, `sys_clk` **+0.410 ns**; all setup/hold domains clean |
| Logic use | 7,761/8,064 LEs (96%); 4,821 registers |
| Generator divider | 24-bit `REG_GEN_BAUD`; feature-advertised through metadata |
| Current SOF | `0x0050ADC8`, programmed 2026-08-27 |
| Hardware regression | 383/383 full suite; 10/10 smoke; 37/37 browser capture matrix |
