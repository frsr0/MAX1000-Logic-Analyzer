        ...
        if data:
            for i in range(len(data) // 4):
                if data[i * 4:(i + 1) * 4] != b'\x00\x00\x00\x00':
                    data = data[i * 4:]
                    break
        # FIX (Bug 3): dead code that followed the 'return data' above has been
        # removed.  It was unreachable and silently discarded the chained_read
        # retry loop that was meant to handle partial bursts.
        return data
