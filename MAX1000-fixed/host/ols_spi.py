...
class OLS:
    ...
    def tx(self, cmd, data=None):
        """SPI transaction.  If data is None, sends cmd alone (1 byte, no padding).
        If data is provided (bytes, length 1-4), sends cmd+data as one CS-low burst.
        Returns received bytes (same length as sent).

        FIX (Bug 4): the original default data=b'\\x11\\x11\\x11\\x11' caused the
        FPGA to receive four extra 0x11 bytes (CMD_XON) after every short command,
        re-arming the capture mid-readout and corrupting sample data.  Short
        commands (CMD_ARM, CMD_RESET, etc.) now send exactly 1 byte.
        """
        if data is None:
            n = 1
            payload = bytes([cmd])
        ...
        self.dev.write(bytes([0x31, n - 1, 0x00]))
