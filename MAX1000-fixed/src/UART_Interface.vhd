...
ARCHITECTURE BEHAVIORAL OF UART_Interface IS
  ...
  -- Clamp to minimum 1 (prevents 0 TO -1 range errors at high baud rates)
  function clamp1(v : integer) return integer is
  begin
    if v < 1 then return 1; else return v; end if;
  end function;
  ...
  -- Dynamic OS_Rate: limit to what the clock can support.
  -- actual_os_rate = min(OS_Rate, CLK_Frequency / Baud_Rate), minimum 1.
  constant max_possible_os : integer := CLK_Frequency / Baud_Rate;
  constant actual_os_rate  : integer := clamp1(imin(OS_Rate, max_possible_os));

  -- Clamp counter divisors to minimum 1
  constant baud_clocks : integer := clamp1(CLK_Frequency / Baud_Rate);
  constant os_clocks   : integer := clamp1(CLK_Frequency / (Baud_Rate * actual_os_rate));
...
BEGIN
  ...
  CLK_GENERATOR : PROCESS (CLK)
    VARIABLE count_baud :   INTEGER RANGE 0 TO baud_clocks-1 := 0;
    VARIABLE count_os   :   INTEGER RANGE 0 TO os_clocks-1 := 0;
  ...
  IF RISING_EDGE(CLK) THEN
    IF (Reset = '1') THEN
      ...
      count_os := 0;
    ELSE
      IF (count_baud < baud_clocks-1) THEN
  ...
  RECEIVE_PROCESS : PROCESS (CLK)
    ...
    VARIABLE os_count   :   INTEGER RANGE 0 TO actual_os_rate-1 := 0;
    ...
    ELSIF (os_pulse = '1') THEN
      CASE (rx_state) IS
        WHEN idle =>
          RX_Busy <= '0';
          IF (RX = '0') THEN
            IF (os_count < actual_os_rate/2) THEN
              os_count := os_count + 1;
              rx_state <= idle;
            ELSE
              os_count := 0;
          ...
          END IF;
        WHEN receive =>
          IF (os_count < actual_os_rate-1) THEN
            os_count := os_count + 1;
            rx_state <= receive;
