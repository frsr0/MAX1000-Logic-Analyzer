...
BEGIN
  ...
  -- FIX (Bug 2): In SPI mode the FSM handshake is:
  --   FSM sets UART_TX_Data then UART_TX_Enable='1'  → effective_TX_Busy goes '1'
  --   FSM waits for effective_TX_Busy='0'            → safe to advance to next byte
  --   FSM waits for effective_TX_Busy='1'            → (skipped in SPI path)
  --
  -- The original code cleared effective_TX_Busy on SPI_RX_Valid (a receive
  -- event), which is unrelated to whether the slave has loaded the new TX_Data
  -- into its shift register.  This caused the SPI slave to clock out the
  -- PREVIOUS byte's data while the FSM had already moved on to the next one —
  -- a systematic one-sample lag on every SPI readout.
  --
  -- Correct behaviour: effective_TX_Busy clears one cycle after UART_TX_Enable
  -- is asserted (TX_Data is already stable by then).  The SPI slave consumes
  -- TX_Data on the next sck_fall after bit 7 rises, which is always later.
    ...
    if rising_edge(CLK) then
      if interface_mode_i = '1' then
        if UART_TX_Enable = '1' then
          ...
          -- SPI slave's TX_Data port and is stable.  The slave latches it on
          -- the next sck_fall (guaranteed to arrive later at any SPI rate
          -- supported by the sys_clk oversampling).
          effective_TX_Busy <= '0';
        end if;
      else
