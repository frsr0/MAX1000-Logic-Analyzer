...
The SPI slave loads the preamble (status byte) into the TX shift register on CS falling edge. The preamble is the first MISO byte of each transaction. Subsequent MISO bytes come from the TX FIFO or UART_TX_Data fallback.
